...
def eval_genomes(genomes, config):
    ...
    while run and len(games) > 0:
        ...
        for x, game in enumerate(games):
            ...
            lr = 1
            if gen % 5 == 0 and len(pastGamesStates) != 0:
                print(len(pastGamesStates))
                for i in range(100):
                    #print("prebackprop")
                    nets[games.index(game)].backProp_Genome(pastGamesStates[0][0:10], pastGamesStates[1][0:10], lr,ge[x])
                    print(i/len(pastGamesStates))
                print("\n the batch acc is")
