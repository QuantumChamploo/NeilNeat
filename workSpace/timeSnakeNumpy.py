...
def eval_genomes(genomes, config):
    ...
    while run and len(games) > 0:
        ...
        for x, game in enumerate(games):
            ...
            if gen % 5 == 0 and len(pastGamesStates[0]) > batchSize:
                batchTime1 = time.time()
                print("about to start 10 sized training loop on all examples")
                for i in range(25):

                    nets[games.index(game)].backProp_GenomeTimed2(pastGamesStates[0][0:batchSize], pastGamesStates[1][0:batchSize], lr,ge[x])
